...
MACRO(LINK_SYSTEM_LIBRARY _TARGET _NAME)
  IF(NOT TARGET ${_NAME})
    ...
    ELSEIF(_IS_SHARED)
      IF(APPLE)
        SET(CMAKE_FIND_LIBRARY_SUFFIXES ".tbd" ${CMAKE_SHARED_LIBRARY_SUFFIX} ".so")
      ELSE()
...
MACRO(SET_TARGET_EXECUTABLE _TYPE name)
  ...
  IF(WIN32 AND WINDOWS_RESOURCES_DIR)
    IF(_IDLS AND NMAKE)
...
MACRO(SET_TARGET_LIB name)
  ...
  FOREACH(ARG ${ARGN})
    IF(_BUNDLE_LABEL STREQUAL "NEXT")
      SET(_BUNDLE_LABEL ${ARG})
    ELSEIF(_ARCH STREQUAL "required")
      SET(_ARCH ${ARG})
  ...
  IF(CMAKE_OSX_ARCHITECTURES AND TARGETS_COUNT GREATER 1)
    ...
    IF(_SHARED_LIB_TARGET)
      ...
      ADD_CUSTOM_COMMAND(TARGET ${_SHARED_LIB_TARGET} POST_BUILD
        COMMAND lipo -output ${CMAKE_BINARY_DIR}/lib/lib${_OUTPUT_NAME_RELEASE}.dylib -create ${_OSX_LIBRARIES})
    ENDIF()
    ...
    IF(_STATIC_LIB_TARGET)
      ...
      ADD_CUSTOM_COMMAND(TARGET ${_STATIC_LIB_TARGET} POST_BUILD
        COMMAND lipo -output ${CMAKE_BINARY_DIR}/lib/lib${_OUTPUT_NAME_RELEASE}.a -create ${_OSX_LIBRARIES})
    ENDIF()
  ELSE()
    IF(_SHARED_LIB_TARGET)
      COMPILE_LIB(${_SHARED_LIB_TARGET} SHARED ${_SOURCES_SHARED})
      SET_TARGET_PROPERTIES(${_SHARED_LIB_TARGET} PROPERTIES OUTPUT_NAME_DEBUG ${_OUTPUT_NAME_DEBUG} OUTPUT_NAME_RELEASE ${_OUTPUT_NAME_RELEASE})
    ENDIF()

    IF(_STATIC_LIB_TARGET)
      COMPILE_LIB(${_STATIC_LIB_TARGET} STATIC ${_SOURCES_STATIC})
      IF(NOT WIN32)
        SET_TARGET_PROPERTIES(${_STATIC_LIB_TARGET} PROPERTIES OUTPUT_NAME_DEBUG ${_OUTPUT_NAME_DEBUG} OUTPUT_NAME_RELEASE ${_OUTPUT_NAME_RELEASE})
      ENDIF()
    ENDIF()
  ENDIF()
...
#      ADD_PLATFORM_FLAGS("-Wno-invalid-command-line-argument -Wno-unused-command-line-argument")
    ...
    IF(NOT XCODE)
      ...
      IF(WITH_CPP11 AND CMAKE_CXX11_EXTENSION_COMPILE_OPTION)
        SET(USE_CPP11 ON)
        SET(PLATFORM_CXXFLAGS "${PLATFORM_CXXFLAGS} ${CMAKE_CXX11_EXTENSION_COMPILE_OPTION}")
      ENDIF()
    ELSE()
