...
MACRO(INIT_BUILD_FLAGS)
  ...
  MESSAGE(STATUS "DEB_HOST_ARCH_CPU $ENV{DEB_HOST_ARCH_CPU}")
  
  # Determine target CPU
  IF(NOT TARGET_CPU)
    IF($ENV{DEB_HOST_ARCH_CPU})
      MESSAGE(STATUS "DEB_HOST_ARCH_CPU 1 $ENV{DEB_HOST_ARCH_CPU}")
      SET(TARGET_CPU $ENV{DEB_HOST_ARCH_CPU})
    ELSE($ENV{DEB_HOST_ARCH_CPU})
      MESSAGE(STATUS "DEB_HOST_ARCH_CPU 2")
      SET(TARGET_CPU ${CMAKE_SYSTEM_PROCESSOR})
    ENDIF($ENV{DEB_HOST_ARCH_CPU})
  ...
  IF($ENV{DEB_HOST_MULTIARCH})
    SET(CMAKE_C_LANGUAGE_ARCHITECTURE $ENV{DEB_HOST_MULTIARCH})
    SET(CMAKE_CXX_LANGUAGE_ARCHITECTURE $ENV{DEB_HOST_MULTIARCH})
    SET(CMAKE_LIBRARY_ARCHITECTURE $ENV{DEB_HOST_MULTIARCH})
    SET(CMAKE_C_LIBRARY_ARCHITECTURE $ENV{DEB_HOST_MULTIARCH})
    SET(CMAKE_CXX_LIBRARY_ARCHITECTURE $ENV{DEB_HOST_MULTIARCH})
  ENDIF($ENV{DEB_HOST_MULTIARCH})
...
MACRO(FIND_PACKAGE_HELPER NAME INCLUDE RELEASE DEBUG)
  ...
  IF(${ARGC} EQUAL 5)
    SET(SUFFIXES ${ARGN})
  ELSE(${ARGC} EQUAL 5)
    SET(SUFFIXES ${NAME} ${LOWNAME} ${LOWNAME_FIXED})
  ENDIF(${ARGC} EQUAL 5)
